...
export const useProfileActions = (
  ...
  const handleSaveProfileAsync = useCallback(async (
    ...
    const savedStudent: Student = {
      ...
      schoolId: (editingStudent as any)?.schoolId || getActiveSchoolId(), // Ensure schoolId is present
    };

    const activeSchoolId = savedStudent.schoolId;
    console.log(`[useProfileActions] Prepared savedStudent for ${studentId}:`, {
    ...
    showToast(
      editingStudent
        ? `Updating profile for "${formData.studentName}"...`
        : `Saving profile for "${formData.studentName}"...`,
      "info"
    );
    ...
    if (offlineMode) {
      ...
      showToast(
        editingStudent
          ? `Profile for "${formData.studentName}" updated in local cache (pending server sync).`
          : `Profile for "${formData.studentName}" saved to local cache (pending server sync).`,
        "success"
      );
    ...
    try {
      if (editingStudent) {
        ...
        showToast(`Profile for "${formData.studentName}" successfully updated on the server!`, "success");
      } else {
      ...
      showToast(`Server sync failed. Profile saved to local cache for later upload.`, "warning");
      window.dispatchEvent(new CustomEvent("offline-queue-changed"));
    }
